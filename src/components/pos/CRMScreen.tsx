      ...
      <div className="flex gap-4">
        ...
        <div className="flex-[2] bg-card border border-border rounded-2xl p-5 shadow-card">
          <div className="flex justify-between items-center mb-4">
            <div className="text-[15px] font-bold text-foreground">👥 รายชื่อลูกค้า</div>
          ...
          <div className="divide-y divide-border/40">
            {CUSTOMERS.map((c, i) => (
              <div key={i} className="flex items-center gap-3.5 py-3.5 hover:bg-surface-alt/50 -mx-2 px-2 rounded-xl cursor-pointer transition-colors">
                <div className="w-10 h-10 rounded-xl gradient-primary flex items-center justify-center text-white font-extrabold text-[15px] shrink-0 shadow-primary">
                  {c.name.charAt(3)}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[12px] text-muted-foreground truncate">{c.phone} · เมนูโปรด: {c.fav}</div>
                </div>
                <div className="text-right shrink-0">
                  <div className="font-mono text-[14px] font-bold text-accent tabular-nums">฿{c.spent.toLocaleString()}</div>
                  <div className="text-[11px] text-muted-foreground">{c.visits} ครั้ง · {c.lastVisit}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="flex-1 flex flex-col gap-4">
          ...
          <div className="bg-card border border-border rounded-2xl p-5 shadow-card">
            <div className="text-[15px] font-bold mb-4 text-foreground">🎁 โปรแกรมสะสมแต้ม</div>
            <div className="space-y-2">
              {LOYALTY.map((t) => (
                <div key={t.tier} className={`flex items-center gap-2.5 px-3 py-3 rounded-xl border ${
                  ...
                  <div className="flex-1">
                    <div className="text-[13px] font-semibold text-foreground">{t.rule}</div>
                    <div className="text-[11px] text-muted-foreground">{t.reward}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-card border border-border rounded-2xl p-5 shadow-card relative overflow-hidden">
            ...
            <div className="space-y-2 relative">
              {AI_TIPS.map((tip, i) => (
                <div key={i} className="px-3 py-2.5 rounded-xl bg-surface-alt border border-border text-[12px] leading-relaxed text-muted-foreground hover:border-primary/30 cursor-pointer transition-colors">
                  {tip}
                </div>
              ))}
            </div>
